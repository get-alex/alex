...
var processor = unified().use(english).use(equality).use(profanities).use(severity);
...
function render(text) {
  ...
  function all(file) {
    ...
    while (++index < length) {
      ...
      results.push(h('span.offense', {
        key: key++,
        className: offset[2] ? 'error' : 'warn'
  ...
  function messages(file) {
    ...
    return [
      ...
      h('.issue-summary', {key: 'summary'}, [
        ...
        h('span.counts', [
          h('span.count', {className: stats.fatal ? 'error' : ''}, String(stats.fatal)),
          h('span.count', {className: stats.nonfatal ? 'warn' : ''}, String(stats.nonfatal)),
          h('span.count', '0')
        ])
      ])
    ];
